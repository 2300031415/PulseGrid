...
import { Users, Settings, ArrowRight, ClipboardList, Shield } from "lucide-react";
...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    // Read doctor details from local storage
    ...
    if (stored) {
      try {
        ...
        setDoctorInfo({
          ...
          specialty: parsed.specialtyOrDepartment || "Cardiologist",
          hospitalName: parsed.hospitalName || "City General Hospital",
        });
      } catch {}
    }

    // Load patient diagnostic stats
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[28px] border border-slate-200 bg-gradient-to-r from-slate-950 via-slate-900 to-teal-950 p-6 text-white shadow-[0_18px_50px_rgba(15,23,42,0.12)] lg:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-bold">Welcome back, {doctorInfo.name}</h2>
            <p className="mt-3 max-w-2xl text-slate-200">
              Manage patient registries, assign clinical lab tests, and review pathology reports from your dashboard.
            </p>
          </div>
          <div className="rounded-3xl border border-white/10 bg-white/10 p-4 text-sm text-slate-200">
            ...
            <p className="mt-1">{doctorInfo.specialty} • Attending Rounds</p>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Link
          href="/dashboard/doctor/patients?filter=Pending"
          className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition group"
        >
          <div className="flex items-center justify-between">
        ...
        <Link
          href="/dashboard/doctor/patients?filter=Uploaded"
          className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition group"
        >
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-slate-50 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <span className="font-bold text-slate-800 text-sm block">Attending Access Active</span>
                <span className="text-xs text-slate-500 font-medium mt-1 block">
                  You are authorized to review active patient diagnostic logs and assign lab report panels.
                </span>
              </div>
            </div>
