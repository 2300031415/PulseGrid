              ...
              return (
                <div key={patient.id} className="p-5 space-y-4">
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <h3 className="font-bold text-slate-900 text-base leading-tight">{patient.name}</h3>
                      <p className="text-xs text-slate-400 font-bold mt-1">
                        ID: {patient.id} {patient.productId ? `• Product: ${patient.productId}` : ""}
                      </p>
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-teal-600 bg-teal-50 px-2.5 py-1 rounded-md border border-teal-100 shrink-0">
                      {patient.ward}
                    </span>
                  </div>

                  <div className="flex items-center justify-between text-xs font-semibold text-slate-500 gap-4 border-t border-slate-50 pt-3">
                    <div>Age: <span className="text-slate-800 font-bold">{patient.age}</span></div>
                    <div className="flex flex-wrap gap-1 justify-end max-w-[200px]">
                      {tests.length === 0 ? (
                        <span className="text-[10px] text-slate-400 font-semibold bg-slate-100 px-2.5 py-0.5 rounded-lg">
                          No tests assigned
                        </span>
                      ) : (
                        tests.map((test) => (
                          <span
                            key={test.id}
                            className={`inline-flex items-center gap-0.5 text-[9px] font-bold px-2 py-0.5 rounded-lg border ${
                  ...
                  <div className="flex gap-2 pt-3 border-t border-slate-50 text-xs font-bold w-full">
                    <button
                      onClick={() => openManageLabTests(patient)}
                      className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-700 hover:bg-teal-50 hover:text-teal-700 transition text-center"
                    >
                      Manage Tests
                    </button>
                    ...
                    {hasUploaded && (
                      <button
                        ...
                        className="flex-1 py-2.5 rounded-xl bg-teal-50 text-teal-700 hover:bg-teal-100 transition text-center"
                      >
                        Review Reports
                      </button>
                    )}

                    <Link href={`/dashboard/doctor/patients/${patient.id}`} className="flex-1">
                      <span className="block py-2.5 rounded-xl bg-slate-900 text-white hover:bg-slate-800 transition text-center animate-micro-scale">
                        Vitals Stream
                      </span>
                    </Link>
                  </div>
